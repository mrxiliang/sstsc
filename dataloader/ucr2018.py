    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            for _ in range(self.K):
                img_transformed = self.transform(img.copy())
                img_list.append(self.totensor_transform(img_transformed))
                (img_transformed_P, img_transformed_c,img_transformed_F) = self.transform_cuts(img_transformed.copy())

                img_list_past.append(self.totensor_transform(img_transformed_P))
                img_list_cen.append(self.totensor_transform(img_transformed_c))
                img_list_future.append(self.totensor_transform(img_transformed_F))

        else:
            img_list = img

        return img_list, img_list_past,img_list_cen, img_list_future, target
...
class UCR20181_for_pi(data.Dataset):
...
class MultiUCR2018_WS(data.Dataset):
    """Override torchvision CIFAR10 for multi-image management.
    Similar class can be defined for other datasets (e.g. CIFAR100).
    Given K total augmentations, it returns a list of lenght K with
    different augmentations of the input mini-batch.
    ...
    def __init__(self, data, targets, K, transform, totensor_transform, transform_w,transform_s):
        self.data = np.asarray(data, dtype=np.float32)
        self.targets = np.asarray(targets, dtype=np.int16)
        self.K = K  # tot number of augmentations
        self.transform = transform
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            img_transformed = self.transform(img.copy())
            img_list.append(self.totensor_transform(img_transformed))
            img_transformed_w=self.transform_w(img_transformed.copy())
            img_transformed_s=self.transform_s(img_transformed.copy())
            img_list_w.append(self.totensor_transform(img_transformed_w))
            img_list_s.append(self.totensor_transform(img_transformed_s))
        else:
            img_list = img
        return img_list, img_list_w, img_list_s, target

    def __len__(self):
        return self.data.shape[0]

class MultiUCR2018_W(data.Dataset):
    """Override torchvision CIFAR10 for multi-image management.
    Similar class can be defined for other datasets (e.g. CIFAR100).
    Given K total augmentations, it returns a list of lenght K with
    different augmentations of the input mini-batch.
    ...
    def __init__(self, data, targets, K, transform, totensor_transform, transform_w):
        self.data = np.asarray(data, dtype=np.float32)
        self.targets = np.asarray(targets, dtype=np.int16)
        self.K = K  # tot number of augmentations
        self.transform = transform
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            img_list = img
        return img_list, img_list_w, target
